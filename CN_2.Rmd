...
#-----------------------------------------------------
# Not R: BEDTOOLS COVERAGE
#-----------------------------------------------------

# Additional information about usage bedtools coverage => calculate per base coverage of all amplicons

# commandline
# File with amplicon regions
# amplFile=sorted.unique.complete.PolypSeq-withAMPL.bed 

# for fileName in *.bam; do output=`echo "$fileName" | sed s/bam/cov/g`; echo "$output"; coverageBed -abam "$fileName" -b $amplFile -d | awk '{print $1"_"$2"_"$3,$8}' > per.base.coverage/"$output"; done;
