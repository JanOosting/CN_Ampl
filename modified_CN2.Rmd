...
library(dplyr)
library(tidyr)
library(gtools) # mixedorder => correct sorting
library(quantsmooth)
...
# amplFile=/home/wplugge/R/additional.pool/alpha_reformat_CHP-rmprimers.bed


#for fileName in *.bam; do coverageBed -abam "$fileName" -b $amplFile -d |  awk -v x="$fileName" '{print x,$1,$2,$3,$4,$5,$6}' >> base.coverage.panel_453.txt ; done;
```
...
outputFileTable="/home/wplugge/R/additional.pool/453/453_CNV_analysis.txt"
#pool.coverage=read.table("/home/wplugge/R/additional.pool/452/base.coverage.panel_452.txt")
pool.coverage=read.table("/home/wplugge/base.coverage.panel_453.txt")
colnames(pool.coverage)=c("sample", "chr", "ampl.start", "ampl.stop", "amplicon", "index", "coverage")

#test=pool.coverage[grep("")]
...
# Checked for overlapping amplicons as well. works
...
#plot(density(avg_stdev_complete$average.stdev))
...
#jan suggestion x_density[which(diff(diff(y_density)>0)==1)] see email. if values are +1 / -1 indicates an approximate position
...
# List with positions is used as index numbers to retrieve corresponding y_density values. To achieve this, a start position (1) and
# end position (nrow(y_density)) are added to this list.
positions=append(positions, 1, after=0) 
positions=append(positions, nrow(y_density), after=length(positions)) # last position

# Calculate the sum of y_density values for each peak to determine which peak is the largest.

curve_sum=list()
for(i in 2:length(positions)){
  curve_sum[as.character(positions[[i]])]=sum(y_density$Y[(positions[[i-1]]+1):positions[[i]]])
}

# identify the peak with the biggerst area. == most samples have this range of average.stdev. other
# peaks are considered outliers
x_density_threshold=x_density[unlist(positions[which.max(curve_sum)+1])] # +1 because first pos is start of curve and added by us into the positions list so we do not want to include this number
...
# samples that have a standard deviation (of the mean) bigger than the calculated threshold (x_density_threshold) are not included in determining the median
...
CI = data.smoothing %>% 
  ...
  #mutate(LCI=q_median+qnorm(0.025)*q_sd, UCI=q_median+qnorm(0.975)*q_sd) %>%
  mutate(LCI=q_median+qnorm(0.005)*q_sd, UCI=q_median+qnorm(0.995)*q_sd) %>%
  group_by(gene) %>%
...
Only samples with a stdev < threshold were included in determining the confidence interval per gene. 
...
facetcond=CI[which(CI$sample.consider=="discarded"),] # provides background color of discarded samples
...
plot(density(as.numeric(avg_stdev_complete$average.stdev)), main="", xlab="average coefficient of variation (per gene(stdev/mean)/mean of sample)")
