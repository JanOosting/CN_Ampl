...
Ampl_within_sample_normalization=function(df){
  df.temp=df[,(which(colnames(bp_med_ampl)=="amplicon")+1):ncol(df)] # Select only columns with (normalized) coverage values
...
Ampl_between_sample_normalization=function(df, sample_sheet){
  ...
  if (length(which(sampleSheet$V2=="Normal"))==0){        # In case where no control samples (normals) are present, divide amplicon values by the 
      ...
      normals=sampleSheet[which(sampleSheet$V2=="Normal"),1] # Retrieve the names of normal samples.
